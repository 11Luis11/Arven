...
import { Search, ShoppingBag, Plus, Minus, Trash2, CheckCircle, AlertTriangle, UserPlus, HelpCircle, ArrowLeft, ShieldAlert, Pencil, X, ChevronDown, Phone, Calendar, Clock, Printer, Download } from 'lucide-react';
...
export default function POS() {
  ...
  const downloadPdfDirectly = async (sale) => {
    try {
      const blob = await generatePdfBlob(sale);
          ...
          <div className="ticket-print-container" style={{
            ...
            <div style={{
              ...
              <div style={{ textAlign: 'center', marginBottom: '16px' }}>
               ...
               {/* Date & Time Icons */}
              <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', fontSize: '10px', color: '#4B5563', backgroundColor: '#F9FAFB', padding: '6px 12px', borderRadius: '4px', border: '1px solid #E5E7EB', marginBottom: '12px' }}>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}><Calendar size={12} /> {saleDate.toLocaleDateString('es-PE')}</span>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}><Clock size={12} /> {saleDate.toLocaleTimeString('es-PE', { hour: '2-digit', minute: '2-digit', hour12: true })}</span>
              </div>
              ...
              <div className="no-print" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button onClick={() => window.print()} className="btn-primary" style={{ flex: 1, display: 'flex', gap: '6px', justifyContent: 'center', borderRadius: '4px', fontSize: '12px', padding: '10px', alignItems: 'center' }}>
                ...
                {/* PDF Download Button */}
                <button
                  onClick={() => downloadPdfDirectly(lastCreatedSale)}
                  className="btn-secondary"
                  style={{
                    width: '100%',
                    padding: '10px',
                ...
                >
                  {sendingWhatsApp ? 'Generando PDF...' : 'Enviar por WhatsApp'}
                </button>
              </div>
            </div>
          </div>
            ...
            <div style={{
              ...
              <div style={{ padding: '24px 28px' }}>
                ...
                <div style={{
                  ...
                  <CheckCircle size={15} /> COMPROBANTE EMITIDO ELECTRONICAMENTE
                </div>
                ...
                <div className="no-print" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  ...
                  >
                    <Download size={15} /> Descargar PDF del Comprobante
                  </button>
